...
#!/usr/bin/env bash
case "$*" in
  *"search prs"*) echo '[{"repository":{"nameWithOwner":"nonrational/lizzie"},"number":131,"isDraft":false,"author":{"login":"nonreagent"}}]';;
...
```
Expected: a log line `... nonrational/lizzie#131 review=9 APPROVED by=stranger -> NOTIFY`, and `state/nonrational__lizzie__131.seen` now contains `9` (marked seen after the notify). No tmux window opened.

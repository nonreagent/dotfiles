...
**Date:** 2026-08-09 \
**Status:** Implemented — `overlay/skills/triage-sweep/` \
...
`./test/run.sh` after the rebuild: 10 passed, 0 failed. The manifest carries the three new rows; overlay↔home copies diff clean; no upstream name collision. Real-world validation is the first unattended run against a live backlog.
